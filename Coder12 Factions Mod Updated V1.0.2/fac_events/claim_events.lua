...
function factions.can_claim_parcel(name, parcelpos)
    ...
    if fn and fn.safezone then
        return false
    end

    if fn and fn.warzone then
        return false
    end
...
--! @brief unclaim a parcel, update power and update global parcels table
function factions.unclaim_parcel(name, parcelpos)
	local parcel = factions.parcels.get(parcelpos)

	-- Safezones are not owned by normal factions
	if parcel and parcel.safezone then
...
-- Safezone / Warzone support
local SAFEZONE_NAME = "safezone"
local WARZONE_NAME = "warzone"

factions.safezone_faction = {
	name = SAFEZONE_NAME,
	allies = {},
	access = {players = {}, factions = {}},
}

factions.warzone_faction = {
	name = WARZONE_NAME,
	allies = {},
	access = {players = {}, factions = {}},
}

function factions.claim_safezone(parcelpos)
	local existing = factions.parcels.get(parcelpos)
...
function factions.claim_warzone(parcelpos)
	local existing = factions.parcels.get(parcelpos)
...
function factions.is_safezone(parcelpos)
	local data = factions.parcels.get(parcelpos)
	return data and data.safezone == true
end

function factions.is_warzone(parcelpos)
	local data = factions.parcels.get(parcelpos)
	return data and data.warzone == true
end
...
function factions.get_parcel_faction(parcelpos)
	...
	if data then
		if data.safezone then
			return factions.safezone_faction, SAFEZONE_NAME
		end

		if data.warzone then
			return factions.warzone_faction, WARZONE_NAME
		end
...
function unclaim_helper(player, faction, parcelpos, no_msgs)
	...
	-- Safezones are not normal factions
	if parcel and parcel.safezone then
		factions.remove_key(factions.parcels, parcelpos, nil, "faction", true)
		return true
	end
